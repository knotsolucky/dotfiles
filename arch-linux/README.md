...
Use this folder to install **packages** that match the apps configured in `~/dotfiles`. Dotfiles for `~/.config` live under **[`../config/`](../config/)**; **`install.sh` runs [`../scripts/sync-all-config.sh`](../scripts/sync-all-config.sh)** after packages so **every** item under `config/` is copied into `$XDG_CONFIG_HOME` (merge into per-app dirs; not a curated subset). Files under **[`../home/`](../home/)** (e.g. **`.zshrc`**) are copied into **`$HOME`**. Use **`chsh -s /bin/zsh`** once if zsh should be your login shell. You can also use **GNU Stow** via [`../scripts/restow-config.sh`](../scripts/restow-config.sh) (symlinks into `~/.config` / `~` instead of copies).
...
After packages, **`install.sh`** runs **`mariadb-install-db`** only when `/var/lib/mysql/mysql` is missing, then **`systemctl enable --now`** for **`NetworkManager`**, **`bluetooth`**, **`docker`**, **`mariadb`**, and **`systemctl --user enable --now syncthing`**. It runs **`elephant service enable`** and **`systemctl --user start elephant`** when **`elephant`** is on `PATH` (from **`elephant-all`**); otherwise it prints a short warning.

Hyprland can still start **Walker** / **Elephant** from **`exec-once`** in **`config/hypr/hyprland.conf`**. Other user units (**`pipewire`**, **`hypridle`**, polkit, portal, etc.) are not enabled by this script — use the manual block below if you want them.

**MariaDB:** if **`enable --now mariadb`** fails, check **`journalctl -u mariadb`**. Re-runs skip **`mariadb-install-db`** when the data directory is already initialized.
...
| `config/wlogout/` | AUR / `wlogout` (power menu; Waybar `custom/power` opens it) — [`../documentation/wlogout.md`](../documentation/wlogout.md) |
| `config/swaync/` | `swaync` |
| `config/kitty/`, `config/alacritty/`, `config/ghostty/` | matching terminal packages (each keeps its **own** font in-repo); UI stack elsewhere uses **JetBrainsMono Nerd** — **`ttf-jetbrains-mono-nerd`** + optional AUR **`nerd-fonts-sf-mono`** for SF terminal fonts ([`../documentation/fonts.md`](../documentation/fonts.md)) |
...
| `config/pipewire/` | `pipewire`, `wireplumber`; AUR **`pwrate`** + [`../documentation/pipewire-pwrate.md`](../documentation/pipewire-pwrate.md) |
