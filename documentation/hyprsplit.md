...
**Arch (`arch-linux/install.sh`):** after syncing config, the script runs `hyprpm update`, `hyprpm add` / `enable` for hyprsplit, and `hyprpm reload -n`. Log back into Hyprland (or `hyprctl reload`) if the session was already running.
